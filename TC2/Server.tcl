...
#.nginx reload
#.nginx status
#.nginx server add <domain> [directory] [+php | +upstream <keyword> <url>]
			...
			if {$subcommand != "" && $domain != "" && [tc2:isdomain $domain]} {
				...
				switch $subcommand {
					add {
						...
						close $fd2
						puts $fd ""
						puts $fd [string map [list %REQUESTER% $requester %TIME% [unixtime] %COMMENT% "Autogenerated by $username" %FULLDOMAIN% $fulldomain %LOGDIR% $logdir %SUBDOMAIN% $subdomain %WWWDIR% $wwwdir %PORT% $port %CUSTOM-PREPHP% "" %CUSTOM-PHP% "" %CUSTOM% "" %UPSTREAMKEYWORD% $upstream_keyword %UPSTREAMURL% $upstream_url] $template]
						close $fd
